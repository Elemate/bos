...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath}/funtionAction_findMenu',
			type : 'POST',
			dataType : 'text',
		...
		$("#btnEp").click(function(){
		    ...
		    if (v){
		        ...
		        $.post("userAction_editPassword",{"password":pwd}, function (data) {
					if (data==1){
					    //修改成功,关闭窗口
						alert(data)
					    $("#editPwdWindow").window("close");
					} else {
                        $.messager.alert("警告","密码修改失败","error");
					}
                });
			}
		});
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/userAction_invalidate.action';
			}
		});
	}
	// 修改密码
                ...
                /*$.extend($.fn.validatebox.defaults.rules, {
                    ...
                    phone: {
                        ...
                        validator: function(value, param){
                            ...
                            var reg = /^1[3|4|7|5|8][0-9]{9}$/;	//手机号码验证规则
				...
				<form id="editPasswordForm" method="post">
					<div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
						<table cellpadding=3>
							<tr>
								<td>新密码：</td>
								<td><input data-options="validType:'length[4,6]'"   required="required" id="txtNewPass" type="Password" class="txt01 easyui-validatebox" /></td>
							</tr>
							<tr>
								<td>确认密码：</td>
								<td><input validType="checkPwd['#txtNewPass']" required="required" id="txtRePass" type="Password" class="txt01 easyui-validatebox" /></td>
							</tr>
						</table>
					</div>
