...
<script type="text/javascript">
	...
	function doAssociations(){
	    ...
	    if (rows.length!=1){
	        $.messager.alert("提示","请选中要关联的定区","warning")
		} else {
            $('#customerWindow').window('open');

            //每次点击关联定区之前清空append的内容
            $("#noassociationSelect").empty();
            $("#associationSelect").empty();

            //获取已经关联定区的客户
            var url = "decidedzoneAction_findListHasAssociation";
            ...
            $.post(url,{"id":decidedzoneId}, function (data) {
                ...
                var mydata = array.join(",")
                $("#rawCustomerIds").val(mydata);
            })

			//获取未关联定区的客户
            url = "decidedzoneAction_findListNotAssociation";
	...
	function doDblClickRow(rowIndex, rowData){
	    ...
	    //定区id
	    var decidedzoneId = rowData.id;
		$('#association_subarea').datagrid( {
			fit : true,
			border : true,
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_associationSubarea?id="+decidedzoneId,
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			rownumbers : true,
			striped : true,
			url : "decidedzoneAction_findListHasAssociation?id="+decidedzoneId,
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="saveDecidedzoneForm" action="decidedzoneAction_save" method="post">
				...
				<table class="table-edit" width="80%" align="center">
					...
					<tr>
						<td>
							...
							<input type="hidden" name="rawCustomerIds" id="rawCustomerIds" />
